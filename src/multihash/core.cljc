...
(defn encode
  ...
  (let [code               (:code mhash)
        length             (:length mhash)
        code-varint-size   (varint/varint-size code)
        length-varint-size (varint/varint-size length)
        encoded            (bytes/byte-array (+ length code-varint-size length-varint-size))]
    (varint/put-varint code encoded 0)
    (varint/put-varint length encoded code-varint-size)
...
(defn decode-array
  ...
  (let [code               (varint/byte-array->varint encoded)
        code-varint-size   (varint/varint-size code)
        length             (varint/byte-array->varint encoded code-varint-size)
        length-varint-size (varint/varint-size length)
...
#?(:clj
   (defn- read-stream-digest
     ...
     ^bytes
     [^InputStream input]
     (let [length (varint/input-stream->varint input)]
       (when-not (pos? length)
         (throw (ex-info
...
(extend-protocol Decodable
  ...
  #?@(:clj
      [InputStream
       ...
       (decode
         [source]
         (let [code   (varint/input-stream->varint source)
